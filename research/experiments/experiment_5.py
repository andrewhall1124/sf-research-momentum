...
def create_summary_table(
    ...
    output_file = Path(f"results/experiment_5/{'vol_scaled_' if vol_scale else ''}combined_table").with_suffix(".png")
    ...
    table = (
        ...
        .tab_source_note(
            source_note=f"Vol Scaled: {'Yes' if vol_scale else 'No'}"
        )
        .tab_source_note(source_note=f"Period: {START_DATE} to {END_DATE}")
        .tab_source_note(
            source_note="Rebalance Frequency: Monthly"
        )
...
def create_returns_chart(
    ...
    file_path = Path(f"results/experiment_5/{'vol_scaled_' if vol_scale else ''}combined_chart")
