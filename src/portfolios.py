...
def construct_portfolios(data: pl.DataFrame, config: Config) -> pl.DataFrame:
    ...
    portfolios = (
        data
        .with_columns(
            pl.col(config.signal.name).qcut(quantiles=config.n_bins, labels=labels).cast(pl.String).over('date').alias('bin')
        )
    )
    ...
    if config.weighting_scheme == 'equal':
        ...
        return (
            portfolios
            .with_columns(
                pl.col('market_cap').truediv(pl.col('market_cap').sum()).over('bin', 'date').alias('weight')
            )
        )
