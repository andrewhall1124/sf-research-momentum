...
# Utility functions
def generate_decile_chart(df_cumulative_returns: pl.DataFrame, name: str, n_bins: int) -> None:
...
assets = (
    ...
    .sort('barrid', 'date')
    .with_columns(
        pl.col('return', 'specific_return').truediv(100)
    )
    .with_columns(
        pl.col('return').shift(-1).over('barrid').alias('fwd_return'),
        pl.col('return').log1p().rolling_sum(window_size=22).exp().sub(1).shift(-22).alias('fwd_return_22d')
    )
)
...
for signal in signals:
    df_portfolios = (
        df_signals
        .with_columns(
            pl.col(signal.__name__).rank(method='random', seed=42).qcut(n_bins, labels=labels).cast(pl.String).over('date').alias('portfolio')
        )
    )
    ...
    if weighting == 'equal':
        ...
        df_returns = (
            df_portfolios
            .group_by('date', 'portfolio').agg(
                pl.col(fwd_return).mul(pl.col('market_cap').truediv(pl.col('market_cap').sum())).sum().alias('return'),
            )
        )
    ...
    df_returns = (
        ...
        .with_columns(
            pl.col(long).sub(pl.col(short)).alias('spread')
        )
    )
