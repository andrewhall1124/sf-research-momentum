...
def time_series_momentum() -> pl.Expr:
    ...
    return ((momentum - rolling_mean) / rolling_std).over('barrid').alias(time_series_momentum.__name__)

def fifty_two_week_high_momentum() -> pl.Expr:
    price = pl.col('return').add(1).cum_prod().sub(1) # I use the cumulative return as a proxy to adjust for splits
    fifty_two_week_high = price.rolling_max(window_size=252)
    return price.truediv(fifty_two_week_high).shift(22).over('barrid').alias(fifty_two_week_high_momentum.__name__)
...
def frog_in_the_pan_momentum() -> pl.Expr:
    ...
    return (momentum * continuousness).over('barrid').alias(frog_in_the_pan_momentum.__name__)

def volatillity_scaled_momentum() -> pl.Expr:
    momentum = pl.col('return').log1p().rolling_sum(window_size=230).shift(22)
    volatillity = pl.col('return').log1p().rolling_std(window_size=230).shift(22)
    return momentum.truediv(volatillity).over('barrid').alias(volatillity_scaled_momentum.__name__)

def idiosyncratic_momentum() -> pl.Expr:
    return pl.col('specific_return').log1p().rolling_sum(window_size=230).shift(22).over('barrid').alias(idiosyncratic_momentum.__name__)
